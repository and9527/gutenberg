...
describe( 'actions', () => {
	...
	describe( 'replaceBlocks', () => {
		it( 'should not yield the REPLACE_BLOCKS action if the replacement is not possible', () => {
			...
			expect(
				replaceBlockGenerator.next( 'core/test-chicken' ),
			).toEqual( {
				value: undefined,
				done: true,
			} );
		...
		it( 'should not yield the REPLACE_BLOCKS if the block being replaced and the replacement are of the same type', () => {
			const blocks = [ {
				clientId: 'ribs',
				name: 'core/test-block',
			} ];
